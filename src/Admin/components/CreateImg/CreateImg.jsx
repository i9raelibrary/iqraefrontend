...
const CreateImg = ({ Articledata, setArticledata }) => {
  const [image, setImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(Articledata.image);
  const [imageName, setImageName] = useState('');
  const [valid, setValid] = useState(true); // Pour vérifier si l'image est valide
  useEffect(() => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      if (validExtensions.includes(fileExtension)) {
        ...
        setValid(false); // L'image n'est pas valide
        setImage(null);
        setImageName('');
